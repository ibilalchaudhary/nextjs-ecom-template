...
const HOST = 'https://api.brandcity.com.pk'
...
const postMyAddress = async (Token, addressObject) => {
  InfoLogger('postMyAddressToken', Token)
  InfoLogger('postMyAddressAddressObject', addressObject)
  SpecialHEADER1.Authorization = `Bearer ${Token}`
  let JsonConverted = null
  await AXIOS.post(addMyAddress, addressObject, {
    headers: SpecialHEADER1,
  })
    .then(response => {
      const { status, data } = response
      if (status === 200 || status === 201) {
        InfoLogger('postMyAddressResponse', data)
        JsonConverted = data
      } else {
        InfoLogger('postMyAddressStatus', status)
      }
    })
    .catch(e => {
      InfoLogger('postMyAddressError', e)
    })
  return JsonConverted
}
...
const sendOrder = async (Token, payload) => {
  SpecialHEADER1.Authorization = `Bearer ${Token}`
  InfoLogger('Sending Orders Object', payload)
  InfoLogger('Token at time of Sending Order', Token)
  ...
  //   headers: SpecialHEADER1
  // })
  await AXIOS.post(placeOrder, payload, {
    withCredentials: true,
    headers: SpecialHEADER1,
  })
    .then(response => {
      const { status, data } = response
      if (status === 200 || status === 201) {
        InfoLogger('sendOrderResponse', data)
        Result = data
      } else {
        InfoLogger('sendOrderResponseWithWrongStatus', response)
      }
    })
    .catch(e => {
      InfoLogger('sendOrderError', e)
    })
  return Result
}
...
const updateMyCart = async (Payload, token) => {
  InfoLogger(AddorRemoveCartCONST, JSON.stringify(Payload))
  InfoLogger('Token at time of updateMyCart', token)
  let JsonConverted = null
  await AXIOS.post(AddorRemoveCartCONST, JSON.stringify(Payload), {
    ...
    const { status, data } = response
    if (status === 200 || status === 201) {
      InfoLogger('updateMyCart Response: ', data)
      JsonConverted = data
    }
  })
  return JsonConverted
